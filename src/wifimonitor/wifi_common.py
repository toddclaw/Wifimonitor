...
@dataclass
class DeauthSummary:
    """Aggregated deauth/disassoc statistics for a single BSSID.

    Generated by :func:`classify_deauth_events` from raw
    :class:`DeauthEvent` objects.  The ``severity`` field indicates
    the threat level based on frame count thresholds.
    """

    bssid: str              # lowercase MAC
    total_count: int        # total frames targeting this BSSID
    broadcast_count: int    # frames with DA = ff:ff:ff:ff:ff:ff
    unique_targets: int     # distinct non-broadcast DAs
    severity: str           # "normal", "suspicious", or "attack"
